...
| 4 | **Your first view** | **C2: views & `@Route`** — "where does my code go." The view renders on a bare page for now — the app shell arrives in ch. 13 | "Hello"-level view at a URL |
...
| 13 | **UI Shell and Navigation** | **C9: an app shell is a layout that wraps every view** — `@Layout` on a class extending AppLayout, SideNav for the menu; C3 composition revisited at app scale, and the first work outside the view class. Hard-coded nav item — "more views = more entries" said, not built | App gains its frame: logo, app name, and side navigation around the same view |
...
| **Vaadin icons** | Cheap and visual — e.g. icons on the New/Delete buttons (ch. 11) or as part of the ch. 14 polish pass; the ch. 13 shell already uses two (`vaadin-h` logo, `users` nav icon) |
...
| 9 | **App shell (`@Layout`)** | A layout class that wraps every view: `@Layout` on a class extending AppLayout, SideNav for the menu. Composition (C3) revisited at app scale — the frame is just another component tree, built in a different class than the view. Kept simple: hard-coded nav item. | ch. 13 |
...
- **`@Menu` / dynamic menus** — out of scope; the ch. 13 shell uses one hard-coded `SideNavItem`. "Views can register themselves in the menu automatically" is at most one spoken sentence.
...
- [ ] Ch. 13 shell build stays simplified: `@Layout` + AppLayout, `vaadin:vaadin-h` logo + app name, one hard-coded SideNav item — no `@Menu`/MenuConfiguration, no DrawerToggle
...
- [ ] No master-detail crept back in; persistence stays toured-not-built; app stays single-view
