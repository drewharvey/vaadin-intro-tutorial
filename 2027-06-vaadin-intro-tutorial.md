...
| 4 | **Your first view** | **C2: views & `@Route`** — "where does my code go." Built in the starter's empty `HomeView`; renders on a bare page for now — the app shell arrives in ch. 13 | "Hello"-level view at a URL |
| 5 | **Components & layouts** | **C3: components are UI elements, layouts arrange them, `add()` composes, layouts nest** — nesting made concrete: two buttons grouped in a HorizontalLayout inside the view's VerticalLayout | TextField + two Buttons laid out on screen |
| 6 | **Listeners: making it interactive** | **C4 + C5: click listener, `getValue()`/`setValue()`, and the "they all look like this" generalization** — Say hello reads the field (`getValue()`), Reset clears it (`setValue("")`) | Click → Notification with the field's value; Reset empties the field |
| 7 | **Display data in a Grid** | **C6: Grid** — the ~3-lines-of-Java credibility moment. C2 revisited: a *new, second* view — `CustomerListView`, `@Route("customers")`, reached by typing the URL until ch. 13 adds navigation. Hardcoded list of the *entity class* used as a plain POJO — so ch. 12 swaps only the data source, never the model | POJO list rendered as a data table, on its own URL |
...
| 13 | **UI Shell and Navigation** | **C9: an app shell is a layout that wraps every view** — `@Layout` on a class extending AppLayout, SideNav for the menu; C3 composition revisited at app scale, and the first work outside a view class. Motivated: two views exist, but `/customers` is reachable only by typing the URL. Hard-coded nav items (Home, Customers) | App gains its frame: logo, app name, and side navigation between both views |
| 14 | **Styling: make it look professional** | — (the visual-payoff chapter, no new core concept): three beats, Java-first. **Beat 1 — polish what we built:** layout spacing/positioning + component theme variants, pure Java. **Beat 2 — theming:** Aura vs Lumo in a sentence, color schemes mentioned not built (`@ColorScheme` annotation, or set it programmatically via `Page::setColorScheme()` — one sentence each, no toggle implemented), and the **Aura theme builder** (Jouni's tool — presets / randomize / customize → copy the generated CSS var overrides: https://vaadin.github.io/web-components/aura.html); the whole app re-skins in one paste. **Beat 3 — how CSS fits (brief):** one small example — the `.app-name` rule in `styles.css` styling the shell's app name (class applied via `addClassName(...)`), deliberately plain CSS values; CSS *variables* get a spoken mention only — enough to understand the var overrides just pasted and future branding; tease the theme-customization video for the full story | Same app, visibly polished; the one-paste re-theme |
...
- **Styling chapter last, three-part structure (ch. 13)** — **(1) Polish what we built**, pure Java: layout spacing/positioning and component theme variants — show how good the app looks with just layouts, Java APIs, and theme variants first; that's the answer to every beginner's first question ("how do I style this?"). **(2) Theming:** Aura vs Lumo positioning in a sentence, color schemes (mentioned only: `@ColorScheme` annotation / programmatic `Page::setColorScheme()` — building a toggle cut for time), and the **Aura theme builder** (https://vaadin.github.io/web-components/aura.html — presets, randomize, customize → copy/paste the generated CSS variable overrides) as the low-effort branding on-ramp. **(3) How CSS fits, brief:** CSS is the power tool, and branding via theme-var overrides needs the basics anyway — one small example: the `.app-name` rule styling the shell's app name via `addClassName(...)`, plain CSS values (variables mentioned, not used), nothing more. CSS never becomes the focus of styling; full theming/branding remains the "Brand Your Vaadin App" mini-tutorial's job (tease it here). Placed after the database chapter: the audience is backend Java devs, so validating the UI against a realistic backend outranks polish — styling is the final cherry on top.
...
| 3 | **Components + layouts** | Components are UI elements; layouts arrange them; compose with `add()`. Key nuance: a layout *is* a component, so composition nests — shown concretely by grouping the two buttons in a HorizontalLayout inside the view. | ch. 5 |
...
- [ ] TextField + buttons → Notification scene covers components/layouts including the HorizontalLayout grouping (ch. 5), then click listeners + `getValue()`/`setValue("")` via Say hello and Reset (ch. 6) — one continuous scene across the chapter cut
...
- [ ] Ch. 14 keeps the three-part order: (1) Java-only polish (variants, spacing/positioning) → (2) theming (Aura vs Lumo, color schemes mentioned only — `@ColorScheme` / `Page::setColorScheme()`, no toggle built — theme-builder paste) → (3) brief CSS beat (the `.app-name` rule via `addClassName(...)`, nothing more) — and teases the theme-customization video
